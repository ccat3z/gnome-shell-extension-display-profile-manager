...
// src: https://gitlab.gnome.org/GNOME/mutter/-/blob/bd28581471b43ca9845e308d75c966281c5af63f/src/org.gnome.Mutter.DisplayConfig.xml
...
var displayConfigProxy = new DisplayConfigProxy(
    Gio.DBus.session,
    'org.gnome.Mutter.DisplayConfig',
    '/org/gnome/Mutter/DisplayConfig'
)
...
async function getResources() {
    const data = await new Promise((resolve) => {
        displayConfigProxy.GetResourcesRemote(resolve)
    })
    return {
        serial: data[0],
        crtcs: data[1].map(d => ({
            ...
            width: d[4],
            height: d[5],
            mode: d[6], // currentMode
            transform: d[7], // currentTransform
            possibleTransforms: d[8],
            properties: d[9]
        })),
        outputs: data[2].map(d => ({
            id: d[0],
            winSysId: d[1],
            crtc: d[2], // currentCrtc
            possibleCrtcs: d[3],
            name: d[4],
            modes: d[5],
            clones: d[6],
        ...
        maxScreenWidth: data[4],
        maxScreenHeight: data[5]
    }
}
...
function applyConfiguration({
    ...
    return new Promise((resolve) => {
        log('apply display configurent: ' + JSON.stringify([
            serial,
            persistent,
            crtcs.map(crtc => [crtc.id, crtc.mode, crtc.x, crtc.y, crtc.transform, crtc.outputs, crtc.properties]),
            outputs.map(output => [output.id, output.properties]),
        ]))
        displayConfigProxy.ApplyConfigurationRemote(
            serial,
            persistent,
            crtcs.map(crtc => [crtc.id, crtc.mode, crtc.x, crtc.y, crtc.transform, crtc.outputs, crtc.properties]),
            outputs.map(output => [output.id, output.properties]),
...
function addMonitorsChangedListener(callback) {
    const id = displayConfigProxy.connectSignal('MonitorsChanged', callback)
    return () => {
        try {
            displayConfigProxy.disconnectSignal(id)
        } catch (e) {
            logError(e)
        }
    }
